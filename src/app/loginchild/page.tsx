      ...
      <form className="relative flex flex-col p-8 gap-4 text-center">
        ...
        <div className="overflow-hidden rounded-md bg-white mt-4 mb-2 w-full">
          ...
          <input
            type="guardcode"
            className="bg-none border-0 outline-0 h-10 w-full border-b border-[#eee] text-sm p-2"
            placeholder="GuardianCode"
          />
        </div>
        <Link href="/main" className="px-4 py-2  text-white rounded">
        <button onClick={() => selectRole('child')} className="bg-[#0066ff] text-white rounded-full py-2 px-4 text-lg font-semibold hover:bg-[#005ce6] transition-all duration-300">
          Sign up
        </button>
        </Link>
      </form>
